...
area <- 0.00556 #m2
E <- 195*10^9 #Pa
inertia <- 0.00000707289 #m4

modal_ii <- data.frame(frequencias[["tramo4_montante_inclinado_Cabo_Montante"]]);colnames(modal_ii) <- c("modo","f")
dados <- modal_ii


force_cable_estimator(modal_ii,mass,length,area,E,inertia)
...
force_cable_estimator<-function(dados,mass,length,area,E,inertia) {
  ...
  fit <- nls(f~newFunc(n=n,L=L,tension,g=g,m=m,A=A,I=I,E=E),start = list(tension=100000000))
